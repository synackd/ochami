...
func newCmdBootConfigPatch() *cobra.Command {
	...
	var bootConfigPatchCmd = &cobra.Command{
		...
		Short: "Patch an existing boot configuration",
		Long: `Patch an existing boot configuration using various patch formats.
...
See ochami-boot(1) for more details.`,
  ...
  ochami boot config patch boo-914afad2 --patch-method rfc6902 --data '[
    {"op":"replace","path":"/kernel","value":"http://s3.openchami.cluster/kernels/vmlinuz2"},
    {"op":"replace","path":"/params","value":"ip=dhcp"},
    {"op":"add","path":"/groups/-","value":"group3"}
  ]'

  # Patch specific fields using JSON merge patch (RFC 7386) (simple merge)
  ochami boot config patch boo-914afad2 --patch-method rfc7386 --data '{"kernel":"http://s3.openchami.cluster/kernels/vmlinuz2","priority":43}'

  # Patch specific fields using dot notation for keys (shorthand patch)
  ochami boot config patch boo-914afad2 --patch-method keyval --set kernel=http://s3.openchami.cluster/kernels/vmlinuz2 --set priority=43 --unset params

  # Patch using payload file
  ochami boot config patch boo-914afad2 -d @payload.json
  ochami boot config patch boo-914afad2 -f yaml -d @payload.yaml
